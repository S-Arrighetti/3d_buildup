...
export const useCargoStore = create<CargoStore>()(
  persist(
    ...
    {
      name: 'buildup-cargo-store',
      version: 1,
      // Colours are handed out round-robin from a module counter; line it back
      // up with the restored items so a reload doesn't repeat the first colours
      onRehydrateStorage: () => (state) => {
        if (state) colorIndex = state.items.length;
      },
    }
  )
);
